...
class RefreshLoadMoreLayout(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (headRefresh != null) {
            ...
            headRefreshHeight = if (headView == null) 0 else headView!!.measuredHeight
        }
        if (footRefresh != null) {
            ...
            footLoadingHeight = if (footView == null) 0 else footView!!.measuredHeight
        }
    }
    ...
    override fun dispatchTouchEvent(ev: MotionEvent): Boolean {
        ...
        when (ev.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                needMove = needControl()
                if (needMove && !isControl) {
                    isControl = true
    ...
    private fun needControl(): Boolean {
        return (!contentView!!.canScrollVertically(-1) && dy > 0 && topScroll) ||
                ((!contentView!!.canScrollVertically(1) && dy < 0 && bottomScroll) ||
                        Math.abs(scrollY) > TOUCH_SLOP ||
                        Math.abs(scrollTop) > TOUCH_SLOP)
    }
    ...
    private fun changeUpOrDown() {
        if (scrollType == TYPE_OVERLAP) {
            if (scrollTop > 0 && headRefresh != null && !isRefreshing) {
                upOrDown(scrollTop, headRefreshHeight, headRefresh!!)
            } else if (scrollTop < 0 && footRefresh != null && !isLoading && !noMore) {
                upOrDown(scrollTop, footLoadingHeight, footRefresh!!)
            }
        } else if (scrollType == TYPE_FOLLOW) {
            if (scrollY < 0 && headRefresh != null && !isRefreshing) {
                upOrDown(scrollY, headRefreshHeight, headRefresh!!)
            } else if (scrollY > 0 && footRefresh != null && !isLoading && !noMore) {
    ...
    private fun upOrDown(scroll: Int, limitHeight: Int, refresh: IRefresh) {
        if (Math.abs(scroll) in 1..limitHeight) {
            ...
            if (state != STATE_DOWN) {
                ...
                refresh.onChangeType(this, IRefresh.TYPE_DOWN)
            }
        }
    }
    ...
    private fun onScrollMove(event: MotionEvent): Boolean {
        if (scrollType == TYPE_FOLLOW) {
            ...
            moveY += dy
                    if (dy > 0) {
                        dy = ((height - contentView!!.top) / height.toFloat() * moveY / 2).toInt()
                    } else {
                        dy = (contentView!!.bottom / height.toFloat() * moveY / 2).toInt()
                    }
            if (dy != 0){
                moveY = 0
            }
    ...
    fun setHeadView(refresh: IRefresh?) {
        ...
        if (refresh != null) {
            refresh.getView(LayoutInflater.from(context), this)
            headView = getChildAt(childCount - 1)
        }
        state = STATE_UP
        if (headRefresh != null) {
            headRefresh!!.onChangeType(this, IRefresh.TYPE_UP)
    ...
    fun setFootView(loadMore: IRefresh?) {
        ...
        if (loadMore != null) {
            loadMore.getView(LayoutInflater.from(context), this)
            footView = getChildAt(childCount - 1)
        }
        state = STATE_UP
        if (footRefresh != null) {
